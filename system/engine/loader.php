...
final class Loader {
	...
	public function extract($model) {
		$file  = DIR_APPLICATION . 'extract/' . $model . '.php';
		$class = 'Extract' . preg_replace('/[^a-zA-Z0-9]/', '', $model);

		if (file_exists($file)) { 
			include_once($file);

			$this->registry->set('extract' . str_replace('/', '_', $extract), new $class($this->registry));
		} else {
